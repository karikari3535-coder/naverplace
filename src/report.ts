...
const TIPS = {
  ...
  npay:"네이버 주문(포장·예약주문)을 연결하면 고객이 방문 전 미리 주문·결제할 수 있어 회전율과 전환율이 올라갑니다. 주문 데이터가 쌓일수록 검색·AI 추천에서도 유리하게 작용합니다. 아직 연결하지 않았다면 스마트플레이스에서 네이버 주문을 신청해보세요.",
...
const ACTIONS = {
  ...
  npay:{todo:"네이버 주문(포장·예약주문)을 연결해보세요. 신청 후 영업일 기준 며칠이면 활성화됩니다.", why:"방문 전 미리 주문·결제가 가능해 회전율이 오르고, 주문 데이터가 쌓이면 노출·AI 추천에 유리합니다."},
